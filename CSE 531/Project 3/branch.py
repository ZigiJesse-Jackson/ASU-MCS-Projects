...
class Branch(banks_pb2_grpc.BranchServiceServicer):
    ...
    def _withdraw(self, amount, request_id):
        ...
        for stub in self.stubList:
            request = banks_pb2.BranchMessage(interface=banks_pb2.Interface.WITHDRAW, request_id=request_id, amount=amount)
            response = stub.Propagate(request)
            if(response.result != banks_pb2.Result.SUCCESS): return False
        return True
    
    def _deposit(self, amount, request_id):
        # creates stubs for communication when stubs haven't yet been initialized
        if(len(self.stubList)==0):
            self.createStub()
        ...
        # communicate to all branches to propogate withdraw query at their site
        for stub in self.stubList:
            request = banks_pb2.BranchMessage(interface=banks_pb2.Interface.DEPOSIT, request_id=request_id, amount=amount)
            response = stub.Propagate(request)
            if(response.result != banks_pb2.Result.SUCCESS): return False
        return True

    def _check_writeset(self, customer_writeset, request_interface):
        # validate writeset from customer request
        if(request_interface == banks_pb2.Interface.QUERY): return True
        customer_writeset = list(customer_writeset)
        
        if(len(customer_writeset)!=len(self.writeset)): return False
        
        for i in range(len(self.writeset)):
            if customer_writeset[i]!=self.writeset[i]: return False
        return True

    def Query(self, request, context):
        # ensure branch writeset is up to date
        if (self._check_writeset(request.writeset, request.interface)==False):
    ...
    def Propagate(self, request, context):
        if(request.interface == banks_pb2.Interface.WITHDRAW): 
            ...
            self.writeset.append(request.request_id)
            return banks_pb2.Reply(interface=request.interface, result=banks_pb2.Result.SUCCESS)
